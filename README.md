...
- 额度窗口展示：按 Codex CLI 返回的实际时长识别 5H、7D 等窗口，并显示剩余百分比与重置时间
- 限额重置明细：悬停额度面板右上角的重置次数，可查看所有可用重置额度及其到期时间
- 托盘额度提示：无需打开面板，即可通过托盘提示和动态图标额度条查看剩余额度
...
- v1.5.0：新增可拖拽额度悬浮卡片，支持启动贴边、悬浮展开、进度显示及托盘菜单开关。
- v1.4.4：新增限额重置明细浮层，可查看全部可用重置额度与到期时间。
- 安装版：支持自动更新。
